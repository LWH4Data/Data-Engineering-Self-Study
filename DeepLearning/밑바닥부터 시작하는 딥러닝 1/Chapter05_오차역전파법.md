...
- 단, 수치 미분과 오차역전파법의 결과 오차가 0이 되는 일은 드물다. 이는 컴퓨터가 할 수 있는 계산의 정밀도가 유한하기 때문이다.

## 7-4. 오차역전파법을 사용한 학습 구현하기
- 지금까지와 다른 점은 기울기를 오차역전파법으로 구한다는 것 뿐이다.
  - 코드는 DL_from_floor/ch05/train_neuralnet.py에 있다.