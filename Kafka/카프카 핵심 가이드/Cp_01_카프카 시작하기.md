...
<ul>
  <li>
    Kafka는 앞서 기술한 특징들로 인해 고부하 아래에서도 높은 성능을 보여준다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka의 코어 프로젝트에는 개발자들이 자주 하는 작업을 수행할 수 있도록 해주는 <strong>플랫폼 기능</strong>이 존재한다.
  </li>
    <ul>
      <li>
        카프카 커넥트는 소스 데이터 시스템으로부터 카프카 데이터를 가져오거나 카프카의 데이터를 싱크 시스템으로 내보내는 작업을 돕는다.
      </li>
      <li>
        카프카 스트림즈는 규모 가변성(scalability)과 내고장성(fault tolerance)을 갖춘 스트림 처리 앱을 쉽게 개발할 수 있게 해주는 라이브러리 이다.
      </li>
        <ul>
          <li>
            가변성(scalability): 시스템의 크기나 처리량을 상황에 따라 유연하게 늘리거나 줄일 수 있는 성질
          </li>
          <li>
            내고장성(fault tolerance):일부 구성 요소에 장애가 발생하여도 시스템 전체가 계속 동작할 수 있는 성질
          </li>
        </ul>
    </ul>
</ul>
...
<ul>
  <li>
    데이터 처리를 위해 구축된 환경에는 많은 앱이 있으며 이를 활용해 Kafka의 작업을 수행할 수 있다.
  </li>
    <ul>
      <li>
        Kafka는 데이터 생태계에 있어 <strong>순환 시스템</strong>을 제공한다.
      </li>
      <li>
        모든 클라이언트에 대해 <strong>일관된 인터페이스</strong>를 제공하면서 다양한 인프라스트럭처 요소들 사이에 메시지를 전달한다.
      </li>
      <li>
        <strong>메시지 스키마</strong>를 제공하는 시스템과 결합하면 프로듀서와 컨슈머는 더 이상 어떤 형태로든 <strong>밀접하게 결합되거나 연결될 필요가 없다</strong>.
      </li>
      <li>
        필요할 때마다 관련 컴포넌트를 추가하거나 제거하면 된다.
      </li>
    </ul>
</ul>
...
<ul>
  <li>
    Linkedin에서 처음 의도한 Kafka의 원래 용도는 <strong>사용자 활동 추적</strong>이었다.
  </li>
  <li>
    <strong>프론트엔드</strong>는 사용자의 행동을 메시지로 생성하여 <strong>하나 이상의 토픽</strong>으로 발행되고, <strong>백엔드</strong>의 애플리케이션에 전달된다. 전달된 메시지는 필요에 따라 <strong>여러 목적으로 소모</strong>된다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka는 <strong>메시지를 교환</strong>하는 데에도 사용할 수 있다.
  </li>
  <li>
    메시지를 전달하는 앱들은 Kafka를 활용하면 메시지 형식이나 전송 방법에 대해 신경 쓸 필요가 없다.
  </li>
    <ul>
      <li>
        같은 룩앤필(look and feel)을 사용해 메시지를 포매팅(formating) 혹은 데코레이팅(decorating)해준다.
      </li>
      <li>
  ...
  <li>
    위와 같은 기능들을 제공하기에 하나의 애플리케이션에서 모든 기능을 처리할 수 있으며 다른 애플리케이션을 사용할 필요가 없어진다.
  </li>
</ul>

<h3>4-1-3. 지표 및 로그 수집</h3>
<ul>
  <li>
    Kafka는 애플리케이션과 시스템의 <strong>지푯값</strong>과 <strong>로그</strong>를 수집하는 데에도 이상적이다.
  </li>
    <ul>
      <li>
        지푯값은 여러 모니터링 툴로, 로그는 엘라스틱 서치 등으로 전달되어 활용될 수 있다.
      </li>
    </ul>
  <li>
    목적 시스템을 변경해야 할 경우 프론트엔드 메시지를 수집 방법을 변경할 필요가 없다는 장점 또한 존재한다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka는 <strong>DB의 커밋 로그</strong> 개념을 기반으로 만들어져 DB에 가해진 변경점들 또한 Kafka로 발행될 수 있다.
  </li>
    <ul>
      <li>
        애플리케이션들은 Kafka로 발행된 스트림을 지켜봄으로써 쉽게 <strong>실시간 업데이트</strong>를 받아볼 수 있다.
      </li>
    </ul>
  <li>
    체인지로그 스트림은 DB에 가해진 업데이트를 <strong>원격 시스템으로 복제</strong>하거나, 다수의 애플리케이션에서 발행한 변경점을 하나의 DB 뷰로 <strong>통합</strong>하는 데에도 사용할 수 있다.
  </li>
  <li>
    컨슈머에 실패가 발생할 경우에도 메시지를 다시 읽어오면 되기에 체인지로그를 저장하는 <strong>버퍼</strong>로서도 유용하게 사용할 수 있다.
  </li>
  <li>
    토픽에 키별로 마지막 값 하나만 보존하는 <strong>로그 압착(compaction)</strong> 기능을 하용하여 로그를 더 오랫동안 보존할 수 있다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka는 <strong>스트림 처리</strong>에 다양하게 활용될 수 있다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka는 링크드인 내부에서의 <strong>데이터 파이프라인 문제</strong>를 해결하기 위해 개발되었다. Kafka는 <strong>다양한 종류의 데이터</strong>를 다루고 <strong>고성능 메시지 교환 시스템</strong> 역할을 할 수 있도록 설계되었다.
  </li>
</ul>

<br>

<h2>5-1. 링크드인이 직면한 문제</h2>
<ul>
  <li>
    링크드인은 폴링 방식의 모니터링을 하였는데 <strong>수집 간격</strong>이 길고, 애플리케이션 <strong>담당자</strong>가 수집된 지표를 관리할 수 없다는 문제가 있었다.
  </li>
    <ul>
      <li>
        간단한 작업도 사람이 직접 수행해야 했으며, 똑같은 측정값이라 하여도 시스템이 다르면 지표 이름까지 달라져 일관성이 없어진다.
      </li>
    </ul>
  <li>
    사용자 활동 정보를 추적하기 위해 HTTP 서비스를 활용했는데 <strong>XML 형식</strong>을 기반으로 한 배치 처리였는데 데이터 형식의 일관성이 없고 파싱하는데 많은 리소스가 소모되는 문제 또한 있었다. 또한 스키마 변경으로 인해 시스템이 중단되기도 하였다.
  </li>
    <ul>
      <li>
        위와 같은 문제들로 인하여 모니터링과 사용자 활동 추적과 같은 백엔드 서비스를 사용할 수가 없었다.
      </li>
    </ul>
  <li>
    문제를 해결하기 위해 데이터에 실시간 접근이 가능하면서 필요한 만큼의 메시지 트래픽을 처리할 수 있을 정도로 <strong>수평 확장(scale out)</strong>이 가능한 새로운 솔루션(ActiveMQ 등)을 시도하였으나 효과가 없었다.
  </li>
  <li>
    최종적으로 링크드인은 데이터 파이프라인을 구축하기 위한 <strong>커스텀 인프라스트럭처</strong>를 자체 개발하는 것을 다음 목표로 하였다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka의 주된 목표는 다음과 같았다.
  </li>
    <ul>
      <li>
        push-pull model을 사용하여 프로듀서와 컨슈머를 <strong>분리(decouple)</strong> 시킨다.
      </li>
      <li>
        <strong>다수의 컨슈머</strong>가 사용할 수 있도록 메시지 교환 시스템의 데이터를 <strong>영속적으로 저장</strong>한다.
      </li>
      <li>
        <strong>높은 메시지 처리량</strong>을 보일 수 있도록 최적화한다.
      </li>
      <li>
        데이터 스트림의 양이 증가함에 따라 시스템을 <strong>수평 확장</strong>할 수 있도록 한다.
      </li>
    </ul>
  <li>
    메시지 직렬화를 위해 Apache Avro와 함께 사용하며 매일 7조 개의 메시지를 쓰고 5페타바이트가 넘는 데이터를 읽을 수 있는 시스템으로 성장하였다.
  </li>
</ul>
...
<ul>
  <li>
    2010말 GitHub에 오픈소스로 공개됨.
    <br>→ 2011년 7월 아파치 소프트웨어 재단의 인큐베이터 프로젝트가 됨.
    <br>→ 2012년 10월 정식 프로젝트가 되어 현재까지 성장 중임.
  </li>
  <li>
    다양한 서드파티 라이브러리가 나오며 여전히 활발하게 개발이 이루어지고 있다.
  </li>
</ul>
...
<ul>
  <li>
    2014년 가을 제이 크렙스와 네하 나크헤데, 준 라오는 링크드인을 떠나 아파치 카프카 개발, 기업 지원, 교육을 전문으로 하는 기업 컨플루언트(confluent)를 창업하여 구글과 AWS 등의 기업에 서비스를 제공하고 있다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka는 쓰기에 최적화된 시스템이기에 작가의 이름을 사용하여 이름을 지었다. 이때 사용한 작가의 이름이 프란츠 카프카(Franz Kafka)이다.
  </li>
</ul>