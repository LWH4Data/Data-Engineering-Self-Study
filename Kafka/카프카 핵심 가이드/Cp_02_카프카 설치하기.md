...
<ul>
  <li>
    다수의 Kafka 브로커가 하나의 <strong>클러스터</strong>를 이루게 하기 위해서는 다음 두 가지를 설정해야 한다.
  </li>
    <ul>
      <li>
        첫 번째는 모든 브로커들이 <strong>동일한 zookeeper.connect</strong> 설정값을 가져야 한다.
      </li>
        <ul>
          <li>
      ...
      <li>
        두 번째는 클러스터 안의 모든 브로커가 <strong>유일한 broker.id</strong> 설정값을 가져야 한다.
      </li>
    </ul>
</ul>
...
<ul>
  <li>
    대부분의 리눅스 배포판은 웬만한 어플리케이션에 잘 작동하도록 <strong>커널 튜닝 매개변수가 미리</strong> 잡혀있다.
  </li>
  <li>
    이 매개변수들은 대개 <strong>/etc/sysctl.conf</strong>에 설정되지만, 자세한 내용은 각자가 사용중인 <strong>리눅스 배포판의 문서</strong>를 참조해야 한다.
  </li>
</ul>
...
<ul>
  <li>
    Kafka는 <strong>스왑 공간</strong>이나 <strong>더티 메모리 페이지(dirty memory page)</strong>가 Kafka의 부하 특성에 맞게 메모리가 사용되는 방식을 조절할 수 있다.
  </li>
  <li>
    대부분의 애플리케이션은 어떻게 해서든 <strong>스와핑</strong>을 막는 것이 최선이다.
  </li>
    <ul>
      <li>
        메모리에서 디스크로 스와핑 될 때에 발생하는 비용은 애플리케이션의 성능을 잡아먹으며 특히 Kafka의 모든 성능 측면에 있어 눈에 띄는 영향을 미친다. 
      </li>
      <li>
        메모리가 부족하여 디스크로 스와핑이 일어나는 경우 페이지 캐시는 <strong>OS의 처리 우선 순위</strong>에서 밀리게 되며 이것이 Kafka에 영향을 준다.
      </li>
    </ul>
  <li>
    스와핑을 방지하는 방법 중 하나는 <strong>아예 스왑 공간 자체를 할당하지 않는</strong> 방법이 있다.
  </li>
    <ul>
      <li>
        다만 스왑 자체는 메모리가 부족할 때 안전 장치 역할을 하기 때문에 실질적으로는 <strong>vm.swappiness</strong> 설정을 <strong>매우 작은 값</strong>으로 하는 것을 권장한다.
      </li>
        <ul>
          <li>
            vm.swappiness는 가상 메모리 서브시스템이 메모리를 페이지 캐시로 사용할지 아니면 스왑 공간으로 사용할지에 대한 <strong>선호도</strong>를 백분율로 나타내는 매개변수이다. <strong>높을수록 스왑공간을 선호</strong>한다.
          </li>
          <li>
            일반적으로 <strong>페이지 캐시</strong>에 더 많은 메모리를 할당해 주는 것이 좋다.
          </li>
          <li>
            여담으로 과거에는 <strong>'swappiness = 0'</strong>이 의미하는 것이 <strong>메모리가 부족하지 않으면 스왑하지 말라는 의미</strong>였으나, 리눅스의 버전이 바뀜에 따라 <strong>어떠한 경우에도 스와핑을 하지 말라는 의미</strong>로 변경되어 0을 사용하지 않는다.
          </li>
        </ul>
    </ul>
  <li>
    Kafka는 프로듀서의 요청에 빠르게 응답하기 위해 <strong>디스크의 입출력 성능</strong>에 의존하며 이로인해 로그 세그먼트가 더 빠른 디스크에 저장된다.
  </li>
    <ul>
      <li>
        이때 <strong>vm.dirty_background_ratio</strong>를 통해 <strong>더티 페이지</strong>의 비율을 조정한다.
      </li>
      <li>
        더티 페이지란 아직 <strong>디스크에 반영되지 않은 변경 내용</strong>이 있는 페이지로 해당 영역을 일정량 보유해야한다.
      </li>
        <ul>
          <li>
            0으로 설정할 경우 더티 페이지를 계속 디스크에 내리려하기 때문에 Kafka의 성능이 떨어진다.
          </li>
        </ul>
      <li>
        많은 경우 5로 잡아주는 것이 일반적이며 0으로 설정하면 안 된다.
      </li>
    </ul>
  <li>
    리눅스 커널이 더티 페이지를 <strong>강제로 디스크에 동기적</strong>으로 내보내기 전에 유지할 수 있는 더티 페이지의 전체 수는 <strong>vm.dirty_ratio</strong>로 증가시킬 수 있다.
  </li>
    <ul>
      <li>
        vm.dirty_ratio의 값은 폭넓게 설정할 수 있지만, 60에서 80사이가 바람직하다.
      </li>
      <li>
        단, vm.dirty_ratio 설정은 아래와 같은 작은 위험을 발생시킨다.
      </li>
        <ul>
          <li>
            <strong>밀린 디스트 쓰기 작업</strong>이 늘어날 수 있다.
          </li>
          <li>
            더티 페이지를 동기적으로 내보내야 할 경우, <strong>I/O 멈춤</strong>이 길어질 수 있다.
          </li>
        </ul>
      <li>
        vm.dirty_ratio 설정값을 올려잡을 경우 Kafka 클러스터의 <strong>복제 기능</strong>을 활성화하여 시스템 장애에 대한 안전장치를 해 놓을 것을 권장한다.
      </li>
    </ul>
  <li>
    앞서 이야기한 매개변수들을 설정할 때에는 <strong>실제 프로덕션 환경</strong>이나 <strong>모의</strong>로 구축한 Kafka 클러스터의 <strong>더티 페이지 수를 모니터링</strong>하는 것이 좋다.
  </li>
    <ul>
      <li>
        더티 페이지 수는 <strong>/proc/vmstat</strong> 파일에서 확인할 수 있다.
      </li>
    </ul>
  <li>
    Kafka는 로그 세그먼트를 저장하고 연결을 열기 위해 <strong>파일 디스크립터</strong>를 사용한다.
  </li>
    <ul>
      <li>
        파일 디스크립터란 OS가 열려 있는 파일(또는 I/O 자원)을 <strong>식별하기 위해 붙여주는 번호</strong>이다.
      </li>
    </ul>
  <li>
    만약 브로커에 많은 파티션이 저장되어 있을 경우,브롴커는 최소한 다음과 같은 수의 파일 디스크립터를 필요로 한다.
  </li>
    <ul>
      <li>
        <strong>{파티션 수} X {{파티션 수} / {세그먼트 크기}} + {브로커에 생성된 네트워크 연결 수</strong>}
      </li>
    </ul>
  <li>
    한 프로세스가 가질 수 있는 메모리 매핑의 최대 개수를 <strong>vm.max_map_count</strong>라 하며 <strong>위의 계산</strong>에 근거하여 충분히 큰 값을 설정하는 것이 좋다.
  </li>
  <li>
    또한 <strong>vm.overcommit_memory를 0</strong>으로 설정하여 <strong>OS의 메모리에 대한 개입을 최소화</strong>하는 것 또한 권장된다.
  </li>
    <ul>
      <li>
        0이 아닌 값으로 할 경우 OS가 많은 메모리를 차지하여 Kafka의 성능이 떨어질 수 있다.
      </li>
    </ul>
</ul>
...
<ul>
  <li>
    하드웨어 다음으로 성능에 큰 영향을 미치는 것은 디스크의 <strong>파일 시스템</strong>이다.
  </li>
  <li>
    사용할 수 있는 파일 시스템은 많지만 일반적으로 <strong>Ext4(fourth extended filesystem)</strong>이나 <strong>XFS(Extents File System)</strong> 중 하나를 로컬 파일시스템으로 사용한다.
  </li>
    <ul>
      <li>
        많은 내용이 함축되어 있지만 결론적으로는 <strong>XFS</strong>가 Ext4보다 더 나은 I/O 처리량을 보여준다.
      </li>
    </ul>
  <li>
    로그 세그먼트를 저장하는 파일 시스템에 무엇을 선택했는지와 별개로 마운트 옵션에는 <strong>noatime</strong> 옵션을 지정해 주는 것이 좋다.
  </li>
    <ul>
      <li>
        atime은 <strong>파일이 읽힐 때마다</strong> 변경되는데 이러한 처리를 Kafka와는 상극이다.
      </li>
      <li>
        atime의 경우 파일이 마지막으로 수정된 이래 사용된 적이 있는지를 알아햐할 때 사용한다.
      </li>
      <li>
        따라서 Kafka는 atime 옵션을 전혀 사용하지 않는다.
      </li>
    </ul>
</ul>
...
<ul>
  <li>
    리눅스 커널은 기본적으로 대량의 데이터를 빠르게 주고받는 데 최적화 되어 있지 않기에 <strong>대량의 네트워크 트래픽</strong>이 발생하는 앱을 위해서는 설정을 조정해야 한다.
  </li>
  <li>
    첫 번째로 조정해야 할 부분은 각 소켓의 <strong>송신, 수신 버퍼에 할당되는 기본/최대 메모리의 양</strong>이다.
  </li>
    <ul>
      <li>
        소켓별 소신 및 수신 버퍼의 기본 크기와 연관이 있는 매개변수는 <strong>net.core.wmem_default</strong>와 <strong>net.core.rmem_default</strong>이며 합리적인 설정값은 131072(128KB)이다.
      </li>
      <li>
        송신 및 수진 버퍼의 최대 크기를 정의하는 매개변수 <strong>net.core.wmem_max</strong>와 <strong>net.core.reme_max</strong>로는 2097152(2MB)가 적절하다.
      </li>
    </ul>
  <li>
    소켓 설정 외에는 <strong>TCP 소켓의 송신, 수신 버퍼 크기(net.ipv4.tcp_wmem, net.ipv4.tcp_rmem)</strong> 역시 별도로 설정해야 한다.
  </li>
    <ul>
      <li>
        TCP 소켓의 매개변수들은 빈으로 구분된 정숫값 세 개(최소 기본 최대)로 설정된다.
      </li>
      <li>
        또한, 최대 크니와 최소 크기는 소켓의 최대 크기보다 클 수 없다.
      </li>
    </ul>
  <li>
    네트워크 연결에 더 많은 버퍼링을 허용하고 싶다면 Kafka 브로커의 <strong>실제 작업 부하</strong>에 근거하여 최댓값을 올려 잡으면 된다.
  </li>
    <ul>
      <li>
        소켓의 최댓값을 올리면 네트워크의 속도는 고정적이기에 더 많은 버퍼링이 허용된다.
      </li>
    </ul>
  <li>
    외에도 net.ipv4.tcp_window_scaling 값을 1로 잡거나, net.ipv4.tcp_max_syn_backlog 설정값을 기본값인 1024보다 크게 잡는 등의 설정이 추가로 존재한다.
  </li>
</ul>

<br><br>

<h1>7. 프로덕션 환경에서의 고려사항</h1>
<h2>7-1. 가비지 수집기 옵션</h2>
<ul>
  <li>
    가비지 수집기는 과거에는 정말 어려운 문제였으나 Java 7에서 <strong>가비지 우선 수집기(Garbage-First garbage collector, G1GC)</strong>가 등장하며 해소가 되었다.
  </li>
  <li>
    Kafka는 G1GC를 기본 가비지 수집기로 사용할 것을 권장한다.
  </li>
    <ul>
      <li>
        G1GC는 서로 다른 작업 부하를 자동으로 조절하고 애플리케이션이 실행되는 동안 <strong>더 일정한 정지 시간(pause time)</strong>을 보여주도록 설계 되었다.
      </li>
      <li>
        G1GC는 <strong>힙을 여러 개의 작은 영역</strong>으로 나누어 처리하여 커다란 크기를 가진 힙 영역을 더 잘 처리한다.
      </li>
    </ul>
  <li>
    GIGC는 최소한의 설정만으로도 모든 것을 수행하며 성능을 조절할 때에는 다음 두 가지 옵션을 활용한다.
  </li>
    <ul>
      <li>
        ...
        <ul>
          <li>
            각 가비지 수집 사이클에 있어 <strong>선호되는 중단 시간</strong>을 지정한다.
          </li>
          <li>
            선호를 의미하기에 고정된 값이 아니며 따라서 <strong>초과</strong>가 될 수도 있다. (기본은 200 밀리초이다).
          </li>
          <li>
            즉, 설정된 밀리초에 맞추어 가비지 수집이 실행되는 영역의 수를 조절한다. 시간을 맞추기 위해 <strong>region을 조절</strong>하기에 영역의 수를 조절하게 된다.
          </li>
        </ul>
      <li>
        <strong>InitiatingHeapOccupancyPercent</strong>
      </li>
        <ul>
          <li>
            G1GC가 수집 사이클을 시작하기 전까지 <strong>전체 힙에서 사용 가능한 비율</strong>을 백분율로 지정한다.
          </li>
          <li>
            기본값은 45이며 이는 전체 <strong>힙의 45%</strong>가 사용되기 전까지 G1GC가 <strong>가비지 수집 사이클을 시작하지 않음</strong>을 의미한다.
          </li>
            <ul>
              <li>
                새 객체가 생성되는 영역인 Eden과 오래된 객체가 저장되는 영역이 모두 포함된다.
              </li>
            </ul>
        </ul>
    </ul>
  <li>
    kafka 브로커는 <strong>힙 메모리를 상당히 효율적</strong>으로 사용하며 가비지 수집의 대상이 되는 <strong>객체 또한 가능한 적게</strong> 생성하기에 위의 값을 낮게 잡아줘도 된다.
  </li>
    <ul>
      <li>
        예를 들어 도서에는 64GB 메모리 서비 + 5GB 크기의 힙에 MaxGCPauseMillis=20ms 그리고 InitiatingHeapOccupancyPercent=35로 설정이 있다.
      </li>
    </ul>
  <li>
    G1GC가 나오기 전의 Kafka는 JVM에 대해 호환을 유지하기 위해 CMS(Concurrent Mark and Sweep) 가비지 수집기를 사용하는데 Kafka의 최신 버전을 사용한다면 <strong>G1GC로 고쳐주어야 한다</strong>.
  </li>
</ul>

<br>

<h2>7-2. 데이터센터 레이아웃</h2>
<ul>
  <li>
    일반적으로 Kafka 브로커의 물리적 위치는 중요하지 않지만, 트래픽을 받아야하는 경우 물리적 위치가 중요해진다.
  </li>
    <ul>
      <li>
        따라서 물리적 위치가 중요한 경우 데이터 센터의 rack에서 차지하는 물리적 위치와 데이터 센터가 <strong>장애 영역(fault zone)</strong> 개념이 있는 환경이 권장된다.
      </li>
    </ul>
  <li>
    Kafka는 브로커에 새 파티션을 할당할 때 랙의 위치를 고려하여 특정 파티션의 레플리카가 <strong>서로 다른 랙</strong>에 배치되도록 할 수 있다.
  </li>
    <ul>
      <li>
        다른 랙에 배치하려면 <strong>broker.rack</strong> 설정을 잡아 주어야 한다.
      </li>
      <li>
        단, 해당 설정은 <strong>새로 생성된 파티션</strong>에 대해서만 적용된다.
      </li>
    </ul>
  <li>
    Kafka 클러스터는 자동으로 랙 인식(rack awareness) 할당을 유지하는지 모니터링 하지 않는다. 따라서 랙 인식 할당을 위해서는 별도의 툴을 사용해야 한다.
  </li>
  <li>
    Kafka 클러스터의 각 브로커가 <strong>서로 다른 랙</strong>에 설치 되도록 하거나 최소한 인프라 측면에서 <strong>단일 장애점(single point of failure)</strong>이 없도록 하는 것이 권장된다.
  </li>
    <ul>
      <li>
        이중 전원(dual power)이나 이중 네트워크 스위치(dual network switch)가 장착된 서버에 설치할 수도 있지만 서로 다른 랙에 배치하는 것이 여전히 권장된다.
      </li>
    </ul>
</ul>
...
<ul>
  <li>
    도서에서도 다루고 있지만 이미 zookeeper 의존도는 지속적으로 줄어들고 있으며 최근에는 <strong>KRaft</strong>를 통해 거의 완전히 벗어난 모습을 보여주고 있다.
  </li>
  <li>
    Kafka는 브로커, 토픽, 파티션에 대한 <strong>메타데이터 정보</strong>를 저장하기 위해 주키퍼를 사용한다.
  </li>
    <ul>
      <li>
        컨슈머 그룹 및 클라이언트는 트래픽이 작기 때문에 굳이 별도의 주키퍼 앙상블을 할당하지 않지만, 만약 많은 경우 <strong>여러 대의 Kafka 클러스터</strong>가 공유해서 사용하도록 할당하기도 한다.
      </li>
        <ul>
          <li>
            위와 같은 때에는 각 클러스터에 대해 <strong>서로 다른 주키퍼 chroot 경로</strong>를 사용한다.
          </li>
        </ul>
      <li>
        컨슈머가 주키퍼와 직접 연관될 수 있으며 이 경우 다양한 옵션을 설정할 수 있다.
      </li>
    </ul>
  <li>
    <strong>다수의 Kafka 클러스터</strong>가 하나의 주키퍼 앙상블을 공유하는 경우를 제외하고서는 <strong>다른 애플리케이션</strong>과 주키퍼 앙상블을 공유하는 것은 권장되지 않는다.
  </li>
    <ul>
      <li>
        앙상블과의 통신에 문제가 생기면 브로커들은 예상치 못한 방향으로 작동할 수 있는데 다른 앱과 공유를 하는 경우 의도치 않게 브로커에 문제가 생길 가능성이 있기 때문이다.
      </li>
    </ul>
</ul>