...
<h1>4. 두 가지의 명확한 기대치 설정</h1>
<h2>4-1. 이 책을 통해서 기대할 수 있는 것</h2>
<ul>
  <li>
    3장 ~ 7장: 순차적이면서 평가 가능한 피드백으로 학습할 수 있는 에이전트
  </li>
    <ul>
      <li>
        테이블 기반(tabular) 강화학습을 배운다.
      </li>
      <li>
        테이블 기반이란 샘플링하는 데이터가 적고, 어떤 신경망이나 함수 근사화가 필요하지 않은 문제를 의미한다.
      </li>
      <li>
        3장: 강화학습과 시간적 가치 할당 문제 간의 연속적인 특성.
      </li>
      <li>
        4장: 두 가지 문제를 분리해서 학습, 평가 가능한 피드백으로 학습할 때 어려운 점을 동시에 다루는 법
      </li>
      <li>
        5장: 정해진 행동에 대한 결과를 평가하는 방법을 학습하는 에이전트
      </li>
      <li>
        6장: 행동을 개선하는 방법을 학습.
      </li>
      <li>
        7장: 강화학습을 좀 더 효율적으로 만들 수 있는 기법
      </li>
    </ul>
  <li>
    8장 ~ 12장: 심층 강화학습 알고리즘과 방법론 그리고 기술들
  </li>
    <ul>
      <li>
        가치 혹은 정책 기반 방법부터 액터-크리틱 방법을 적용한 다양한 에이전트들
      </li>
      <li>
        8장 ~ 10장: 가치 기반 심층강화학습
      </li>
      <li>
        11장: 정책 기반 심층 강화학습과 액터-크리틱
      </li>
      <li>
        12장: Deterministic policy gradien(DPG)와 soft actor-critic(SAC) 그리고 Proximal policy optimization(PPO)
      </li>
    </ul>
  <li>
    13장: 복습
...
<h2>4-2. 이 책의 내용을 얻어가는 방법</h2>
<ul>
  <li>
    간단하게라도 머신러닝과 딥러닝을 학습한 경험이 필요하다.
  </li>
  <li>
    파이썬에 익숙한 독자여야 한다.
  </li>
  <li>
    수학적 부분은 많은 설명을 달아 수학 관련 지식이 없어도 이해할 수 있게 구성하였다.
  </li>
  <li>
    도서의 모든 내용을 보는 것이 중요하다.
  </li>
  <li>
    실습 코드 기반 다양한 시도를 해보는 것을 권장한다.
  </li>
</ul>

<br>

<h2>4-3. 심층 강화학습 개발 환경</h2>
<ul>
  <li>
    실습 환경을 제공해 주기 보다 https://goodboychan.github.io/book 에 있는 코드를 통해 코드 실행에 필요한 사항들을 따라할 것을 권장한다.
  </li>
  <li>
    도서는 주로 넘파이와 파이토치 위주로 구성되어 있다.
  </li>
  <li>
    파이토치나 별도의 심층 강화학습 라이브러리를 만드는 방법에 대해 설명하지는 않지만 도서를 읽기만 해도 충분히 이해할 수 있다.
  </li>
  <li>
    에이전트를 학습하기 위한 환경으로 OpenAI Gym 패키지와 저자가 개발한 라이브러리를 활용하지만 이들은 모두 에이전트를 학습하기 위한 환경을 제공하는 라이브러리라는 점만 인지하면 된다.
  </li>
  <li>
    심층 강화학습의 구조는 딥러닝 모델 정도의 연산 수준을 필요로 하지 않는다. 따라서 정말 안 좋은 하드웨어가 아니라면 도커 컨테이너에서 충분히 실습할 수 있다.
  </li>
    <ul>
      <li>
        RL은 보통 CPU의 멀티 스레드 방법을 사용한다.
      </li>
      <li>
        대안으로는 Google Colabotary에서 무료로 제공하는 딥러닝 하드웨어를 사용할 수 있다.
      </li>
    </ul>
</ul>
...
<ul>
  <li>
    평가 가능한 피드백으로 학습을 하면 에이전트는 <strong>정보 수집과 활용 사이에 균형</strong>을 맞추는 법을 학습한다. 이를 통해 <strong>오래된 경험을 새로운 경험으로 일반화</strong> 할 수 있다.
  </li>
  <li>
    강화학습은 지도학습 그리고 비지도학습과 함께 머신러닝의 세 분야 중 하나이다.
  </li>
  <li>
    심층 강화학습에서 핵십적으로 필요한 요소는 샘플링 복잡성이 좋은 알고리즘, 탐험 전략이 좋은 알고리즘, 그리고 안전한 알고리즘이다.
  </li>
</ul>