...
```python
# 구간 곱 구하기 - 11505
# ♬ 도서에서는 buffer 입출력 까지는 사용안하고 MOD만 사용한다. 
# ♬ 그 외의 풀이는 모두 나에게 맞게 적합하게 템플릿을 변경하였다.
...
<ul>
  <li>
    트리 그래프에서 임의의 두 노드를 선택했을 때 두 노드가 각각 자신을 포함해 거슬러 올라가면서 부모 노드를 탐색할 때 <strong>처음 공통으로 만나게 되는 부모 노드</strong>를 <strong>최소 공통 조상(Lowest common ancestor, LCA)</strong>라 한다.
  </li>
</ul>

<br>

<h2>5-1. 최소 공통 조상의 핵심 이론</h2>
<ul>
  <li>
    1. 일반적인 최소 공통 조상 구하기
  </li>
    <ul>
      <li>
        ...
        <ul>
          <li>
            a-1. 루트 노드에서 탐색을 시작해 각 노드의 <strong>부모 노드</strong>와 <strong>깊이</strong>를 저장한다.
          </li>
          <li>
            a-2. 선택된 두 노드의 깊이가 다른 경우 더 깊은 노드를 부모 노드로 1개씩 올려주면서 같은 깊이로 맞춘다.
          </li>
          <li>
            a-3. 위의 과정을 반복하던 중 <strong>두 노드가 같아진다면</strong> 해당 노드가 최소 공통 조상이다. 
          </li>
            <ul>
              <li>
                a-3-1. <strong>깊이가 같은 상태</strong>에서는 <strong>동시에 부모 노드</strong>로 올라가면서 두 노드가 같은 노드가 될 때까지 반복한다.
              </li>
            </ul>
        </ul>
    </ul>
  <li>
    b. 최소 공통 조상 빠르게 구하기 (a의 방법은 모든 깊이 탐색이기에 깊이가 깊어지면 느리다).
  </li>
    <ul>
      <li>
        알고리즘의 핵심은 기존에 한 칸씩 올리던 방식에서 <strong>2^k</strong>씩 올라가는 방법이다. 따라서 기존에 자신의 부모 노드만 저장하는 방법에서 <strong>2^k 번째 부모 노드</strong> 까지 저장해야 한다.
      </li>
      <li>
        이 방법은 탐색 대상의 노드의 2^K 번째 부모부터 <strong>내려오면서 탐색</strong>하는 방법이다.
      </li>
      <li>
        b-1. 부모 노드 저장 리스트 만들기
      </li>
        <ul>
          <li>
            b-1-1. 부모 노드 리스트의 정의
            ...
            <ul>
              <li>
                P[K][N] = N번 노드의 2^K 번째 부모 노드의 번호
              </li>
            </ul>
          <li>
            b-1-2. 부모 노드 리스트의 점화식
          </li>
            <ul>
              <li>
                P[K][N] = P[K - 1][P[K - 1][N]]
      ...
      <li>
        b-2. 선택된 두 노드의 깊이 맞추기
      </li>
        <ul>
          <li>
            높이의 차이가 나는 만큼 <strong>2^K</strong>를 위로 올라가며 찾는다. 예를 들어 20의 높이 차이가 난다면 2^4(16)과 2^2(4)를 활용하여 찾을 수 있다.
          </li>
        </ul>
      <li>
        b-3. 최소 공통 조상 찾기
        ...
        <ul>
          <li>
            2^K 단위로 올라가며 즉, 부모노드 리스트에서 <strong>K에서 1씩 감소</strong>하며서 최초로 두 노드의 부모가 달라지는 값을 찾는다.
          </li>
          <li>
            <strong>K = 0</strong>일 때까지 반복을 진행한다. 반복이 끝났을 때 두 노드가 같은 노드라면 해당 노드가, 아니라면 바로 위의 부모 노드가 최소 공통 조상이 된다.
          </li>
        </ul>
    </ul>
</ul>
