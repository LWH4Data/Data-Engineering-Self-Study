...
<ul>
  <li>
    기수 정렬(radix sort)은 비교할 <strong>자릿수</strong>를 정한 다음 해당 자릿수만 비교한다.
  </li>
  <li>
    기수 정렬은 <strong>10 개의 큐</strong>를 이용한다.
  </li>
  <li>
    기수 정렬의 시간 복잡도는 <strong>O(kn)</strong>이다. (k: 데이터의 차릿수).
  </li>
  <li>
    <strong>기수 정렬 순서</strong>
  </li>
    <ul>
      <li>
        1. index 별로 queue로 구현된다.
        <br>→ 2. 일의 자리에 맞는 index queue에 해당 수를 삽입한다. (e.g. 0이라면 index 0을 의미하는 queue에 삽입)
        <br>→ 3. 일의 자리 입력이 끝났다면 오름차순 정렬을 수행한다.
        <br>→ 4. 일의 자리로 정렬된 상태에서 십의 자리를 기준으로 다시 index queue에 삽입한다.
        <br>→ 5. 위의 과정을 자리수 만큼 반복하면 끝.
      </li>
    </ul>
</ul>
...
```python
# 수 정렬하기 3 - 10989
#   - 해당 문제에서는 기수 정렬이 아니라 '계수 정렬(counting sort)'을 
#     활용한 index 풀이를 한다.
#   - 계수 정렬은 값의 범위의 크기만큼 배열을 생성하고, 해당 값이 등장하는 경우
#     생성된 배열에서 해당하는 index에 +1을 한다.
import sys
input = sys.stdin.readline
N = int(input())
count = [0] * 10001
...
for i in range(10001):
    if count[i] != 0:
        # 해당 count 리스트의 값만큼 index를 반복하여 출력한다.
