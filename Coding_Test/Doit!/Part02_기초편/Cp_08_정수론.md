...
<ul>
  <li>
    확장 유클리드 호제법의 목적은 <strong>방정식의 해</strong>를 구하는 것이다.
  </li>
</ul>

<br>

<h2>4-1. 확장 유클리드 호제법의 핵심 이론</h2>
<ul>
  <li>
    ax + by = c(a, b, c, x, y는 정수)일 때 <strong>c % gcd(a, b) = 0</strong>인 경우에만 정수해를 갖는다.
  </li>
    <ul>
      <li>
        즉, ax + by = c가 정수해를 갖게하는 c의 최솟값이 <strong>gcd(a, b)</strong>임을 의미한다.
      </li>
    </ul>
  <li>
    유클리드 확장판 원리
    ...
    <ul>
      <li>
        1. 정수해를 위한 <strong>gcd(a, b)</strong>를 설정하고 계산한다.
        <br>→ 2. gcd(a, b)로 유클리드 호제법을 반복하며 <strong>몫</strong>과 <strong>나머지</strong>를 저장하고, <strong>나머지가 0</strong>이 되면 중단한다.
        <br>→ 3. 구한 몫과 나머지를 통해 <strong>x = y'</strong>, <strong>y = x' - y' * q</strong>를 역으로 계산한다. (y': 이전 y, x': 이전 x, q: 현재 보고 있는 몫).
        <br>→ 4. 처음 x와 y는 이전 값이 없기에 <strong>x = 0</strong>과 <strong>y = 1</strong>로 하고 계산을 시작한다.
        <br>→ 위의 과정으로 얻어낸 최종 x, y는 ax + by = gcd(a, b)를 만족한다.
        <br>→ c / gcd(a, b) = K를 가정하면 방정식의 해는 Kx, Ky로 간단히 구할 수 있다.
        <br>→ 이후 K를 각 x와 y에 곱해주면 최초 방정식의 해를 구할 수 있다.
      </li>
    </ul>
  <li>
    우변의 값이 gcd(a, b)의 배수가 아니라면 방정식의 해가 정수 범위에 존재하지 않는다.
  </li>
</ul>
...
def Execute(a, b):
    ...
    # 바로 최대공약수가 나오는 경우가 아니라면
    # 유클리스 호제법을 진행하며 최초해를 계산
    q = a // b
    v = Execute(b, a % b)
    ret[0] = v[1]
    ret[1] = v[0] - v[1] * q
...
# 전제 조건 확인을 위한 기본 유클리드 호제법 수행
mgcd = gcd(a, b)

# c가 최대공약수로 나누어지는지 확인.
# 만약 조건 만족이 안 된다면 성립되지 않으므로 -1을 출력
if c % mgcd != 0:
    print(-1)
# 조건 만족인 경우
else:
